...
void Matrix::setPixelColor(uint16_t pixel, uint32_t color) {
    this->colors[pixel] = color;
}

uint32_t Matrix::Color(uint8_t r, uint8_t g, uint8_t b) {
    return ((uint32_t)r << 16) | ((uint32_t)g << 8) | b;
}

// https://github.com/adafruit/Adafruit_NeoPixel/blob/master/Adafruit_NeoPixel.cpp#L3190-L3287
uint32_t Matrix::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
    uint8_t r, g, b;
